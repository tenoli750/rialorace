-- New write RPC for bet placement.
-- Use a new function name to avoid stale PostgREST cache/overload metadata
-- that can run the old create_bet_with_login_session in a read-only tx.

create or replace function public.create_bet_with_login_session_v2(
...
grant execute on function public.create_bet_with_login_session_v2(
  text,
  integer,
  text,
  text,
...
where n.nspname = 'public'
  and p.proname in ('create_bet_with_login_session', 'create_bet_with_login_session_v2')
order by signature;
