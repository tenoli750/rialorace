...
import { RaceEngine } from "./src/raceEngine.js?v=3";
...
import { RaceAudioController } from "./src/raceAudio.js?v=2";
...
const MARKET_01_EDITOR_STORAGE_KEY = "binance-ring-rally-market-01-editor-tuning-v5";
const MARKET_ID = "market-01";
document.querySelector(".sidebar-panel h2").textContent = "Market 01 Result History";

const engine = new RaceEngine({ autoRestart: false, coins: TEST_MARKET_01_COINS });
let ui;
...
const feed = new SupabasePriceFeed(engine, TEST_MARKET_01_COINS, {
...
async function resolveOfficialResult() {
  ...
  if (result.ok) {
    ...
    ui.settleBet(engine);
    engine.addNote("Market 01 podium resolved by Supabase.");
    refreshResultHistory();
  } else {
    resultResolutionInFlight = false;
    engine.addNote(result.message);
...
function loadSavedMarketTuning() {
  try {
    const raw = localStorage.getItem(MARKET_01_EDITOR_STORAGE_KEY);
