import { TEST_MARKET_02_COINS } from "./src/config.js";
...
const MARKET_02_EDITOR_STORAGE_KEY = "binance-ring-rally-market-02-editor-tuning-v3";
const MARKET_ID = "market-02";
document.querySelector(".sidebar-panel h2").textContent = "Market 02 Result History";

const engine = new RaceEngine({ autoRestart: false, coins: TEST_MARKET_02_COINS });
let ui;
...
const feed = new SupabasePriceFeed(engine, TEST_MARKET_02_COINS, {
...
async function resolveOfficialResult() {
  ...
  if (result.ok) {
    ...
    ui.settleBet(engine);
    engine.addNote("Market 02 podium resolved by Supabase.");
    refreshResultHistory();
  } else {
    resultResolutionInFlight = false;
    engine.addNote(result.message);
...
function loadSavedMarketTuning() {
  try {
    const raw = localStorage.getItem(MARKET_02_EDITOR_STORAGE_KEY);
