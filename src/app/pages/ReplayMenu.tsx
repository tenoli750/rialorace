      ...
      <section className="bg-white rounded-lg border border-[#fed7aa] p-6 mb-6">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-3">
          {tokens.map((token) => (
            <div
              key={token.id}
              className="flex items-center gap-3 p-3 bg-[#fff7ed] rounded-md border border-[#fed7aa]"
            >
              ...
              <div className="min-w-0 flex-1">
                <div className="text-sm text-[#9a3412] truncate">{token.symbol}</div>
                <div className="text-xs text-[#8a5a44] truncate">{token.name}</div>
              </div>
            </div>
          ))}
        </div>
